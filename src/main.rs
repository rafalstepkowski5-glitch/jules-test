...
use tower_http::{cors::{CorsLayer}, compression::CompressionLayer, limit::RequestBodyLimitLayer, trace::TraceLayer, services::ServeDir};
...
#[derive(Clone)]
pub struct AppState { pub pool: db::DbPool }

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    dotenvy::dotenv().ok();
    tracing_subscriber::registry().with(tracing_subscriber::EnvFilter::new("info")).with(tracing_subscriber::fmt::layer().json()).init();

    // Early fail-closed security checks for critical environment variables
    let jwt_secret = std::env::var("JWT_SECRET")
        .map_err(|_| anyhow::anyhow!("Krytyczny błąd: Zmienna środowiskowa JWT_SECRET nie została ustawiona!"))?;
