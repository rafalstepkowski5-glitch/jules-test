...
static SECRET: Lazy<String> = Lazy::new(|| {
    let s = std::env::var("JWT_SECRET")
        .expect("Krytyczny błąd: Zmienna środowiskowa JWT_SECRET nie została ustawiona!");
    assert!(
        s.len() >= 32,
        "Zagrożenie bezpieczeństwa: JWT_SECRET musi mieć co najmniej 32 znaki!"
    );
    s
});
...
pub fn create_access_token(u: &str, role: &str) -> String {
    ...
    let c = Claims {
        ...
        iss: "webx-metrics-pro".into(),
        aud: "webx-client".into(),
    };
    encode(
        &Header::default(),
        &c,
...
pub fn verify_jwt(t: &str) -> Result<Claims, String> {
    ...
    decode::<Claims>(t, &DecodingKey::from_secret(SECRET.as_bytes()), &v)
        .map(|d| d.claims)
