...
pub async fn save_metric(
    ...
    sqlx::query("INSERT INTO metrics (ts,cpu,mem,users,rps) VALUES (?,?,?,?,?)")
...
pub async fn get_history(pool: &DbPool, lim: i64) -> anyhow::Result<Vec<MetricPoint>> {
    let rows = sqlx::query("SELECT id,ts,cpu,mem,users,rps FROM metrics ORDER BY id DESC LIMIT ?")
        .bind(lim)
        .fetch_all(pool)
        .await?;
...
pub async fn get_user(pool: &DbPool, u: &str) -> anyhow::Result<Option<UserRow>> {
    if let Some(row) = sqlx::query("SELECT username,password_hash,role FROM users WHERE username=?")
        .bind(u)
        .fetch_optional(pool)
        .await?
    {
...
mod tests {
    ...
    async fn save_metric_and_get_history_roundtrip() {
        let pool = build_test_pool().await;
        save_metric(&pool, 1.23, 4.56, 7.89, 0.12)
            .await
            .expect("save metric");
